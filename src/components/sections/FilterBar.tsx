...
const cuisines: FilterOption[] = [
  { label: 'Todas as Cozinhas', value: 'todas' },
  { label: 'Italiana', value: 'italiana' },
  { label: 'Japonesa', value: 'japonesa' },
  { label: 'Contemporânea', value: 'contemporanea' },
...
export default function FilterBar({ isIfoodActive, setIsIfoodActive, activeSort, setActiveSort, searchQuery, setSearchQuery }: FilterBarProps) {
  const [activeCuisine, setActiveCuisine] = useState<FilterOption | null>(null)
  ...
  // Handlers to simulate selection (in a real app, these would open a modal or standard select)
  // For simplicity and immediate visual feedback without complex popup logic, 
  // we'll use native select overlaid invisibly on the buttons, or just simple click to cycle.
  // Actually, let's implement simple native selects that look like buttons for best accessibility 
  // and ease of use, or custom dropdowns if needed. Custom dropdowns are better for styling.

  const [openDropdown, setOpenDropdown] = useState<'cuisine' | 'price' | 'neighborhood' | 'advanced' | null>(null)

  const toggleDropdown = (dropdown: 'cuisine' | 'price' | 'neighborhood' | 'advanced') => {
      ...
      <div className="container mx-auto px-4 md:px-8 flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative shrink-0">
            ...
            >
              {activeCuisine ? activeCuisine.label : 'Cozinha'}
              <ChevronDown size={16} className={`transition-transform ${openDropdown === 'cuisine' ? 'rotate-180' : ''}`} />
            </button>
            {openDropdown === 'cuisine' && (
              <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-outline/20 rounded-xl shadow-lg py-2 z-50">
                {cuisines.map(c => (
                  <button 
                    key={c.value} 
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-surface transition-colors ${activeCuisine?.value === c.value ? 'text-primary font-bold bg-primary/5' : 'text-on-surface'}`}
                    onClick={() => { setActiveCuisine(c.value === 'todas' ? null : c); setOpenDropdown(null); }}
                  >
                    {c.label}
                  </button>
                ))}
